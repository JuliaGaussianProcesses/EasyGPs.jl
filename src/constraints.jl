...
function flatten(::Type{T1}, p::RealParameter{T2}) where {T1 <: Real, T2 <: Real}
    ...
    unflatten_to_RealParameter(v::Vector{T1}) = RealParameter{T2, typeof(p.par)}(unflatten(v))
...
Base.inv(p::RealParameter{T, <:ParameterHandling.Fixed}) where T <: Real = fixed(inv(value(p)))
...
function fixed(x::Real)
    ...
    RealParameter{typeof(x), typeof(_x)}(_x)
end
